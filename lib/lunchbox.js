...
Lunchbox.prototype.start_order = function(restaurant, invites, func) {
    var self = this;
    this.login(function() {
        ...
        self.request.get(orders_url, function(error, response, body) {
            if (!error && response.statusCode == 200) {
                var order = {};
                order['restaurant_id'] = self._restaurant_id(restaurant, body);
                var time = new Date();
                time.setMinutes(time.getMinutes() + 30);
                order['order_time'] = {};
                order['order_time']['hour'] = time.getHours();
                if (time.getHours() > 12) {
                    order['order_time']['hour'] = time.getHours() - 12;
                }
                order['order_time']['min'] = time.getMinutes();
                order['order_time']['meridian'] = time.getHours() < 12 ? "am" : "pm";
                order['delivery'] = '0';
                order['recipients'] = invites;
            } else {
                self.error("Unable to load the orders page");
            }
        });
    });
};
...
Lunchbox.prototype.orders_since = function(endDate, func) {
    ...
    self.get_order_list(function(body) {
        ...
        $("td").filter(function(i, el) {
            ...
            return self._order_date($(el).text()) >= endDate;
        }).each(function(i, el) {
            ret.push($(el).text().trim());
        });
        func(ret.join("\n"));
