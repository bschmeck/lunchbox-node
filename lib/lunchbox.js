...
Lunchbox.prototype.start_order = function(restaurant, recipient_ids, func) {
    var self = this;
    this.login(function() {
        var orders_url = "http://lunchbox.fm/orders";
        self.request.get(orders_url, function(error, response, body) {
            ...
            var form = {};
            tokens = self._order_tokens(body);
            form['data[_Token][key]'] = tokens['key'];
            form['data[_Token][fields]'] = tokens['fields'];
            ...
            form['data[Order][restaurant_id]'] = restaurantData.id;

            order_time = new Date();
            order_time.setMinutes(order_time.getMinutes() + 45);
            if (order_time.getHours() == 0) {
                form['data[Order][order_time][hour]'] = 12;
            } else if (order_time.getHours() > 12) {
                form['data[Order][order_time][hour]'] = order_time.getHours() - 12;
            } else {
                form['data[Order][order_time][hour]'] = order_time.getHours();
            }
            form['data[Order][order_time][min]'] = order_time.getMinutes();
            if (order_time.getHours() > 11) {
                form['data[Order][order_time][meridian]'] = 'pm';
            } else {
                form['data[Order][order_time][meridian]'] = 'am';
            }

            form['data[Order][delivery]'] = "0";

            if (!self._validate_recipients(body, recipient_ids)) {
                self.error({'message': "Recipient id list invalid (" + JSON.stringify(recipient_ids) + ")"});
                return;
            }
            form['data[Order][recipients]'] = recipient_ids;

            self.request.post({url: orders_url, form: form}, function(error, response, body) {
                if (error || response.statusCode >= 400) {
                    self.error({'message': "Unable to place order (" + response.statusCode + ")",
                                'error': error,
                                'response': response});
                    return;
                }
                func("Order started at " + restaurantData.name);
            });
        });
    });
};
...
Lunchbox.prototype._order_tokens = function(body) {
    ret = {};
    $ = cheerio.load(body);
    ret['key'] = $("input[name='data[_Token][key]']").attr("value");
    ret['fields'] = $("input[name='data[_Token][fields]']").attr("value");
    return ret;
};
...
/*
 * Make sure all the of the given recipient ids are listed on the order form.
 */
Lunchbox.prototype._validate_recipients = function(body, ids) {
    $ = cheerio.load(body);
    var valid_ids = $("#coworker_list > .checkbox > input[type='checkbox']").map(function(i, el) { return $(el).attr('value') } );
    // Want to find out if there's anything in ids that's not in valid_ids
    // Disgusting n**2 algorithm, but the lists are (should be) small and I'm lazy
    var found, i, j;
    found = true;
    for (i = 0; i < ids.length && found; i++) {
        found = false;
        for (j = 0; j < valid_ids.length && !found; j++) {
            found = (ids[i] == valid_ids[j]);
        }
    }
    return found;
};
